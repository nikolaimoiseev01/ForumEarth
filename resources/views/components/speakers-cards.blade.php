<div id="speakers" {{$attributes->merge(['class' => 'py-20 mb-20 bg-blue-500 flex flex-col rounded-t-3xl'])}}>
    <div class="content">
        ...
        <div class="grid grid-cols-3 md:grid-cols-2 sm:!grid-cols-1 gap-8">
            ...
            @if($main_page =='true')
                <a wire:navigate href="{{route('portal.speakers')}}" class="col-span-1 flex flex-col items-center justify-center bg-white rounded-2xl p-6 relative transition hover:scale-[1.01] cursor-pointer">
                    <h2 class="font-bold text-3xl text-blue-600 text-center md:!text-xl">СМОТРЕТЬ СПИСОК ВСЕХ СПИКЕРОВ</h2>
                    <div
                        class="absolute md:hidden bottom-6 right-6 bg-blue-500 w-16 h-16 flex items-center justify-center rounded-xl cursor-pointer transition duration-300 hover:bg-blue-400"
                    >
